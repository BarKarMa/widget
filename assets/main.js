...
socket.on('chat message', (data) => {
  ...
  if (data.name === "server"){
    ...
    //div.className = "alert";
    item.innerHTML = `<p>${data.name}: </p> <span id="my-messages" class="li" >${data.message}</span>`
    messages.appendChild(item)
    window.scrollTo(0,document.body.scrollHeight)
  }
  else {
    const item = document.createElement('div')
    item.innerHTML = `<p >${data.name}:</p> <span id="server-messages"; style="">${data.message}</span>`
